...
//EOS识别的错误代码来自3000000
//请参阅libraries/chain/include/eosio/chain/exceptions.hpp
...
bool print_help_text(const fc::exception& e) {
   bool result = false;
//std:：regex的大输入字符串会导致sigsegv，这是libstdc++中的已知错误。
//长序列见https://stackoverflow.com/questions/36304204/%d0%a1 regex segfault
   auto detail_str = e.to_detail_string();
//2048年，不错的轮数。无特殊原因挑选。报告了22K+字符串的上述错误。
