...
void test_db::idx_double_nan_create_fail(uint64_t receiver, uint64_t, uint64_t) {
   double x = 0.0;
x = x / x; //创建一个
db_idx_double_store( N(nan), N(nan), receiver, 0, &x); //应该失败
}
...
void test_db::idx_double_nan_modify_fail(uint64_t receiver, uint64_t, uint64_t) {
   ...
   x = 0.0;
x = x / x; //创建一个
db_idx_double_update(itr, 0, &x); //应该失败
}
...
/*d test_db：：未对齐的_secondary_key256_tests（uint64_t/*receiver*/，uint64_t，uint64_t）
   auto key=eosio:：key256:：make_from_word_sequence<uint64_t>（0ull，0ull，0ull，42ull）；
   char*ptr=（char*）（&key）；
   PTR+＝1；
   //测试存储区在未对齐数据上不会崩溃
   数据库idx256_store（n（testapi），n（testtable），n（testapi），1，（uint128_t*）（ptr），2）；
   //在未对齐的数据上测试查找主数据不会崩溃
