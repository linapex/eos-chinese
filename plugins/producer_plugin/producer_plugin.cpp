...
uint32_t _production_skip_flags              = 0; //eosio：：chain：：skip_nothing；
      ...
      /*
       *黑客警报
       *Boost计时器可能处于处理程序尚未执行但不可中止的状态。
       *由于此方法需要改变状态处理程序，因此需要依赖于正确的功能来维护
       *其他代码的不变量（即在接近完整的块中接受传入事务）
       *处理程序在设置时捕获一个相互关联ID。当他们被处决时
       *他们必须对照全局序数检查相关性。如果不匹配的话
       *表示此方法是在处理程序应处于的状态下调用的。
       *已取消，但无法取消。
       **/

      uint32_t _timer_corelation_id = 0;

//保持延迟txn和传入txn之间的预期比率
...
//如果这个trx没有失败/软失败，并且设置了persist标志，那么存储它的id以便
//确保它也适用于所有未来的投机区块。
...
//在unix://to http_plugin之后发送整个字符串。它会自动检测哪个部分
//是Unix套接字路径，哪个部分是服务器上要命中的URL
...
uint32_t minimum_offset = 1; //必须至少是“下一个”块

//说明将来将取消此生产商资格的水印
//这是保守的假设，没有掉块。如果删除块，水印将
//取消该生产商的资格更长，但假设他们会醒来，确定他们
//由于跳过块而被取消资格的时间更长，并且用更好的块重新计算下一个块
//然后是信息
...
//这个生产者下一个生产机会是其槽下次到达之后或达到计算的最小值时。
...
if((block_time - now) < fc::microseconds(config::block_interval_us/10) ) {     //我们必须睡至少50毫秒
...
//不管链状态如何，下一秒勾号的计划
//如果等待时间少于50毫秒（块间隔的1/10），则等待整个块间隔。
   const fc::time_point now = fc::time_point::now();
...
//确定我们的水印是否在此时排除我们的生产
   ...
   try {
      ...
      if (_pending_block_mode == pending_block_mode::producing) {
//确定此生成器可以确认的块数
//1）如果不是此节点的生产者，则假定没有确认（我们无论如何都将放弃此块）
//2）如果此节点上的生产商从未生产过，保守的方法是假定
//确认以确保在崩溃后不重复签名：使这些水印耐用？
//3）如果此节点上的生产者知道它生成的最后一个块，则安全地设置它-除非-
//4）此节点最后一个水印上的生产者更高（意思是在另一个分叉上）
         if (currrent_watermark_itr != _producer_watermarks.end()) {
...
//这使我们配置的最大事务时间失败，我们不想重播它，请将其添加到黑名单
...
//如果我们有生产者，那么我们至少应该为下一个可用的时隙设置一个计时器。
...
//ilog（“product_block$t，”，“（t”，fc:：time_point:：now（））；//用于测试_product_time_offset_us
