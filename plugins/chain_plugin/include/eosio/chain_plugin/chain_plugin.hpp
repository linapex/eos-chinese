...
//双重参与率=0；
   ...
   struct get_table_rows_params {
      ...
      string      upper_bound;
      uint32_t    limit = 10;
string      key_type;  //索引位置指定的键类型
string      index_position; //1-主索引（第一个）、2-次索引（按多索引定义的顺序）、3-第三个索引等
string      encode_type{"dec"}; //dec，十六进制，默认值=dec
      optional<bool>  reverse;
optional<bool>  show_payer; //显示RAM
    };

   struct get_table_rows_result {
vector<fc::variant> rows; ///<每个项目一行，编码为十六进制字符串或JSON对象
bool                more = false; ///<true，如果数据中的最后一个元素不是end和sizeof data（）<limit
   };
...
SecKeyType lv = convert_to_type<SecKeyType>( s.to_string(), "lower_bound name" ); //避免编译器错误
...
//支持--key_类型[sha256，ripemd160]和--encoding[dec/hex]
