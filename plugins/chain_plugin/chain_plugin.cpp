...
//声明运算符<<并在与读取模式本身相同的命名空间中验证读取模式的Funciton
namespace chain {
              ...
              int）
{
  ...
  //从“values”中提取第一个字符串。如果有超过
  //一个字符串，这是一个错误，将引发异常。
  std:：string const&s=validators:：get_single_string（值）；

  if（s=“投机”）
     V=Boost:：Any（eosio:：chain:：db_read_mode:：siscular）；
  else if（s=“head”）
     V=boost:：any（eosio:：chain:：db_read_mode:：head）；
  else if（s==“只读”）
     v=boost:：any（eosio:：chain:：db_read_mode:：read_only）；
  else if（s=“不可逆转”）
     V=boost:：any（eosio:：chain:：db_read_mode:：unreversible）；
  }否则{
     抛出验证\u错误（验证\u错误：：无效的\u选项\u值）；
  }
}

std:：ostream&operator<（std:：ostream&osm，eosio:：chain:：validation模式m）
   if（m==eosio:：chain:：validation_mode:：full）
      OSM＜“满”；
   否则，如果（m==eosio:：chain:：validation_mode:：light）
      OSM＜“光”；
   }
...
无效验证（boost:：any&v，
              ...
              eosio:：chain:：validation_模式*/*目标_类型*/,
...
//使用txn_msg_rate_limits=控制器：：txn_msg_rate_limits；
...
//txn_msg_rate_limits rate_limits/速率限制；
...
//TODO:速率限制
         /*“每个授权帐户事务消息速率限制时间帧秒”，bpo:：value<uint32_t>（）->默认值（默认值_per_auth_account_time_frame_seconds），
          “每个授权帐户交易消息速率限制被强制超过的时间范围（以秒为单位）。”
         （“每个授权帐户事务消息速率限制”，bpo:：value<uint32_t>（）->default_value（default_per_auth_account），
          “限制每个授权帐户允许的最大事务消息速率事务消息速率限制时间范围秒。”）
          （“每代码帐户事务消息速率限制时间帧秒”，bpo:：value<uint32_t>（）->默认值（默认值_per_code_account_time_frame_seconds），
           “每代码帐户交易消息速率限制被强制超过的时间范围（以秒为单位）。”
          （“按代码帐户事务消息率限制”，bpo:：value<uint32_t>（）->default_value（default_per_code_account），
           “限制帐户代码允许的每个代码帐户事务消息速率限制时间范围秒的最大事务消息速率。”*/
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
genesis_state gs; //检查eosio根键是否损坏
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   try {
chainbase::database reversible( db_dir, database::read_only); //测试是否脏
//如果它到达这里，那么可逆数据库就不是脏的
...
return false; //因为我们根本不会截断可逆数据库。
   } catch( const std::runtime_error& ) {
   } catch( ... ) {
      throw;
   }
//可逆块数据库已损坏。所以备份它（除非已经移动），然后创建一个新的。
...
ubo.set_block( itr->get_block() ); //获取并设置块而不是复制打包数据作为附加验证
         });
         end = itr->blocknum;
         ++num;
...
fc::raw::unpack(ds, tmp); //验证压缩块是否已损坏。
         reversible_blocks.write( itr->packedblock.data(), itr->packedblock.size() );
         end = itr->blocknum;
         ++num;
      }
...
//返回1——这是programs/nodeos/main.cpp认为的“bad-alloc”
   std::_Exit(1);
}
...
//_uu builtin_popcountll（db.get_dynamic_global_properties（）.recent_slots_filled）/64.0，
      app().version_string(),
   };
}
...
//参见索引名称的多索引打包
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      primary = true;
} else if (starts_with(p.index_position, "sec") || p.index_position == "two") { //二级，二级
} else if (starts_with(p.index_position , "ter") || starts_with(p.index_position, "th")) { //三元、三元、三元、三元
      pos = 1;
} else if (starts_with(p.index_position, "fou")) { //四，第四
      pos = 2;
} else if (starts_with(p.index_position, "fi")) { //五，第五
      pos = 3;
} else if (starts_with(p.index_position, "six")) { //六，第六
      pos = 4;
} else if (starts_with(p.index_position, "sev")) { //七，第七
      pos = 5;
} else if (starts_with(p.index_position, "eig")) { //八，第八
      pos = 6;
} else if (starts_with(p.index_position, "nin")) { //九，第九
      pos = 7;
} else if (starts_with(p.index_position, "ten")) { //十，第十
      pos = 8;
   } else {
      try {
...
if (str.find(',') != string::npos) { //Fix 6274仅匹配格式，如4、EOS
      try {
...
auto end_time = cur_time + fc::microseconds(1000 * 10); ///10ms最大时间
...
//如果我们正在寻找并找到它，则返回false，否则返回true
...
//TODO:将此函数和类似函数移动到头。从wasm_interface.cpp复制。
//TODO:修复严格的别名冲突
...
auto time_limit = fc::time_point::now() + fc::microseconds(1000 * 10); ///10ms最大时间
...
//以下代码对部署在eosio帐户上的合同（即系统合同）及其存储数据的方式进行了假设。
   const auto& d = db.db();
