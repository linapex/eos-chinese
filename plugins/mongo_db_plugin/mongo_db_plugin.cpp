...
///@如果act应添加到mongodb，则返回true；如果为false，则跳过它。
...
void mongo_db_plugin_impl::applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
//从一个不完整的块发出的记录道会将生产者的块ID保留为空。
//
//如果producer_block_id为空，请避免将操作跟踪或事务跟踪添加到数据库中。
//通过这种方式，推测执行的事务的跟踪不会包含在Mongo数据库中，该数据库可以
//避免对该数据库的使用者造成潜在的混淆。
//
//由于forks，可能存在多个具有相同块编号和trx_id的不兼容操作跟踪
//存在于数据库中。如果生产商double生成一个块，即使块时间也可能不会
//消除这两个动作痕迹的歧义。如果没有生产者阻止并确定
//跟踪来自区块链的孤立分支，Mongo DB数据库的消费者可能
//对当前区块链中从未实际执行的过时动作跟踪作出反应。
//
//最好避免这种潜在的混乱，不要记录来自推测性执行的跟踪，即发出的跟踪
//从一个不完整的块。这意味着跟踪不会以推测性读取模式记录，但是
//无论如何，用户不应该在该模式下使用mongo-db-u插件。
//
//如果节点是用于测试的生产者，则允许记录跟踪，因此单个节点可以同时用于测试。
//
//建议以只读模式运行mongo_db_plugin=read-only。
//
...
if( time > fc::microseconds(500000) ) //减少日志记录，.5秒
...
options.projection( make_document( kvp( "_id", 1 )) ); //只有返回身份证
...
//无效键出现逻辑错误，请勿关闭
   ...
   if( shutdown ) {
//如果Mongo未能提供修复问题和重新启动的机会，则关闭
      app().quit();
   }
}

//自定义OID以避免单调节流
//https://docs.mongodb.com/master/core/bulk-write操作/避免单调节流
...
purge_abi_cache(); //必要时腾出空间
               abi_cache entry;
               entry.account = n;
               entry.last_accessed = fc::time_point::now();
               abi_serializer abis;
               if( n == chain::config::system_account_name ) {
//将eosio setabi.abi从字节重新定义为abi_def
//这样，abi在mongo中存储为abi_def，而不是字节。
...
void mongo_db_plugin_impl::process_applied_transaction( const chain::transaction_trace_ptr& t ) {
   try {
//始终调用，因为即使不存储事务跟踪，我们也需要在帐户上捕获setabi
...
const chain::base_action_trace& base = atrace; //没有内联操作跟踪

//在使用mongodb sharding时提高数据分发性
...
bool write_ttrace = false; //过滤器也适用于事务跟踪
...
void mongo_db_plugin_impl::init() {
   ...
   using bsoncxx::builder::basic::make_document;
   using bsoncxx::builder::basic::kvp;
//手动创建本地合同帐户；遗憾的是，我们无法运行它们的合同来让它们自己创建
//请参见本机_contract_chain_initializer:：Prepare_database（）。
