...
namespace eosio {
   ...
   struct account_history_object : public chainbase::object<account_history_object_type, account_history_object>  {
      ...
      id_type      id;
account_name account; ///<在其历史记录中具有此操作的帐户的名称
uint64_t     action_sequence_num = 0; ///<相关操作的序列号（全局）
int32_t      account_sequence_num = 0; ///<此帐户的序列号（每个帐户）
   };
   ...
   struct action_history_object : public chainbase::object<action_history_object_type, action_history_object> {
      ...
      id_type      id;
uint64_t     action_sequence_num; ///<相关操作的序列号
...
chainbase::database& db = const_cast<chainbase::database&>( chain.db() ); //覆盖对状态数据库的只读访问（高度不可通信的做法！）
...
chainbase::database& db = const_cast<chainbase::database&>( chain.db() ); //覆盖对状态数据库的只读访问（高度不可通信的做法！）
//TODO:使用单独的chainbase数据库管理历史\u插件的状态（或完全删除不推荐使用的历史\u插件）
...
( const transaction_id_type &id ) -> bool //十六进制前缀比较
         {
