...
continue; //帐户已被检查为存在，因此其所有者和活动权限应该存在

if( a.permission.permission == config::eosio_code_name ) //虚拟eosio.code权限不存在，但允许
         continue;
...
fc::sha256 code_id; ///default id==0
   ...
   db.modify( account, [&]( auto& a ) {
      /*TODO:考虑微秒级本地时间戳是否足以检测代码版本更改*/
//TODO:更新setcode消息以包含哈希，然后在validate中验证它
...
context.require_authorization(update.account); //仅在此处将此操作的单一权限标记为已使用
...
//如果父级的ID为0将用于指示父级的缺失，那么我们需要确保链
//使用保留ID为0的虚拟对象初始化权限索引。
...
context.require_authorization(remove.account); //仅在此处将此操作的单一权限标记为已使用
...
context.require_authorization(requirement.account); //仅在此处将此操作的单一权限标记为已使用

      auto& db = context.db;
      const auto *account = db.find<account_object, by_name>(requirement.account);
      EOS_ASSERT(account != nullptr, account_query_exception,
"Failed to retrieve account: ${account}", ("account", requirement.account)); //冗余？
...
context.require_authorization(unlink.account); //仅在此处将此操作的单一权限标记为已使用
...
context.require_authorization(cancel.canceling_auth.actor); //仅在此处将此操作的单一权限标记为已使用
