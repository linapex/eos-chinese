...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      if( it == recovery_cache.get<by_sig>().end() || it->trx_id != tid ) {
         ...
         lock.lock();
recovery_cache.emplace_back( cached_pub_key{tid, recov, sig, cpu_usage} ); //在dup签名上可能失败；没有问题
...
decomp.push(read_limiter<1*1024*1024>()); //为保护拉链炸弹，减压至10兆欧。
...
{
//在构造后调用，但始终在同一线程上，并在打包事务传递到任何其他线程之前调用
