...
irreversible = 0, ///<此块以前已被此节点应用，被认为是不可逆的
validated   = 1, ///<这是一个由有效生产者签名的完整块，以前已由该节点应用，因此已验证，但它还不可逆。
complete   = 2, ///<这是一个由有效的生产者签名的完整块，但尚未不可逆转，也尚未被此节点应用
incomplete  = 3, ///<这是一个不完整的块（由生产者生成或由节点推测生成）
         };
         ...
         /*
          *这些交易以前是由推送的，后来被取消应用，调用推送交易
          *如果事务成功，元数据指针将从该数据源中删除它们。
          *
          *调用方负责在失败的事务上调用drop_unapplied_事务
          *他们从不打算重试
          *
          *@返回未应用的交易的向量
          **/
         ...
         /*
          *这些事务ID表示按计划在头链状态下可用的事务。
          *或其他产生的交易。
          *
          *使用这些ID调用push_scheduled_事务将从
          *如果成功或客观失败，则链状态
          *
          *@返回
          **/
