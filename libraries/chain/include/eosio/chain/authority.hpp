...
static const uint64_t value = 24; ///<为了安全，重量超过值
   };

   template<>
   struct billable_size<key_weight> {
static const uint64_t value = 8; ///<重量超过安全值，动态调整尺寸键
   };

   template<>
   struct billable_size<wait_weight> {
static const uint64_t value = 16; ///<超过重量值，等待安全
   };
}
...
keys_size += fc::raw::pack_size(k.key);  ///<键的序列化大小
      }
...
/*
 *确保所有密钥都是唯一和排序的，并且所有帐户权限都是唯一和排序的，并且该权限可以
 *满意
 **/
...
return false; //溢出保护（假设权重类型为uint16，阈值类型为uint32）
...
if( prev && !(prev->key < k.key) ) return false; //TODO:要求键按升序而不是降序排序（需要修改许多测试）
...
if( prev && ( prev->permission >= a.permission ) ) return false; //TODO:要求权限级别按升序而不是降序排序（需要修改许多测试）
