...
//一个简单的突变器，它实际上不会突变任何东西。
//用于验证给定指令是否可在我们的平台上执行
//喷油器“接口”：
//‘kills`->如果这个注入器终止原始指令
//‘post`->此注入器应在原始指令之后注入
   ...
   struct instruction_counter {
      ...
      /*tic uint32_t icnt；/*迄今为止的说明*/
      静态uint32_t tcnt；/*总指令*/

      /*tic uint32_t bcnt；/*来自块类型的指令总数*/
      静态std:：queue<uint32_t>fcnts；
   }；
...
清除，清除
      静态void accept（wasm_ops:：instr*inst，wasm_ops:：visitor_arg&arg）
         指令_counter:：icnt=0；
         block_stack.push（arg.start_index）；
         orderings.back（）.push_back（arg.start_index）；
         bcnt_tables.back（）.emplace（arg.start_index，0）；
         键入_stack.push（inst->get_code（）==wasm_ops:：loop_code）；
      }

      静态std:：stack<size_t>block_stack；
      static std:：stack<size_t>type_stack；/*这可能比将来块是循环捕获更多*/

      /*tic std:：queue<std:：vector<size_t>>orderings；/*记录找到块的顺序*/
      static std:：queue<std:：map<size_t，size_t>>bcnt_tables；/*每个块的表指令计数*/
   ...
   struct checktime_end {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         if ( checktime_block_type::type_stack.empty() ) 
            return;
if ( !checktime_block_type::type_stack.top() ) { //空或不是循环
