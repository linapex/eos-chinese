      ...
      for( const auto& s : structs ) { try {
         if( s.second.base != type_name() ) {
            ...
            while( current.base != type_name() ) {
               ctx.check_deadline();
const auto& base = get_struct(current.base); //<--强制结构从另一个结构继承
   ...
   type_name abi_serializer::resolve_type(const type_name& type)const {
      ...
      if( itr != typedefs.end() ) {
for( auto i = typedefs.size(); i > 0; --i ) { //避免无限递归
...
//问题：是否实际需要行为要求返回的变量不为空？
//这通常不允许选项数组（但如果数组中的所有选项都存在，则允许）。
//是否存在返回的变量将为空的情况，除了空可选变量的情况？
