...
//const auto&usage=_db.get<resource_usage_object，by_owner>（account）；
   /*
    *由于我们需要将这些延迟到下一个资源限制边界，因此它们在“待定”中创建。
    *在现有的“待处理”状态下陈述或调整。链控制器将“挂起”状态折叠为
    *下一个适当边界的实际状态。
    **/
...
//直接更新用户权重
   ...
   if( ram_bytes >= 0 ) {
      ...
      if（limits.ram_bytes<0）
         eos_assert（ram_bytes>=usage.ram_usage，wasm_execution_error，“转换不受限制的帐户将导致过度承诺[commit=$c，所需限制=$l]”，“（c”，usage.ram_usage）（“l”，ram bytes））；
      }否则{
         eos_assert（ram_bytes>=usage.ram_usage，wasm_execution_error，“尝试释放已提交的ram资源[commit=$c，所需限制=$l]”，（“c”，usage.ram_usage）（“l”，ram ram_bytes））；
      }
      **/
