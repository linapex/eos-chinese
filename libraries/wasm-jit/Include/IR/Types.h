...
//WebAssembly运算符结果的类型。基本上与valuetype相同，但允许不带结果的运算符（无）。
...
//大小约束：某些大小约束类型的预期大小范围。
//如果max==uint64_max，则最大大小不受限制。
...
//表包含的元素类型：目前只能是anyfunc，表示任何函数类型。
