...
//进行首次初始化。
		...
		{
//确保没有其他调用growmemory/shrinkmemory的内容。
...
//将numbytes的绝对值舍入到对齐边界，并确保它不会分配太多或太少的内存。
...
default: return stdout;//std:：cerr<“无效文件句柄”<<vmHandle<<std:：endl；throw；
		}
	}
...
//书写
	...
	{
//只有在默认情况下使用emscripten内存时，才将模块初始化为emscripten模块。
...
//调用establishstackspace函数来设置emscripten模块的内部堆栈指针。
