...
namespace eosio { namespace testing {
   ...
   class base_tester {
      public:
         ...
         /*tual signed_block_ptr product_block（fc:：microseconds skip_time=fc:：millises（config:：block_interval_ms），uint32_t skip_flag=0/*skip_missed_block_penalty*/）=0；
         虚拟签名块产生空块（fc:：microseconds skip_time=fc:：millises（config:：block_interval_ms），uint32_t skip_flag=0/*skip_missed_block_刑罚*/ ) = 0;
...
//生成尽可能少的块，以便在不使任何生成者处于非活动状态的情况下花费给定的时间
...
action_result            push_action(action&& cert_act, uint64_t authorizer); //TODO/问题：是否需要？
...
//方法将键视为名称类型，如果这种情况不适合，则传递require==false并报告正确的行为
...
//领域：
      protected:
//tempdir字段必须在control之前，以便在销毁期间，tempdir仅在控制器完成后删除。
   ...
   public:
      ...
      /*Ned_block_ptr product_block（fc:：microseconds skip_time=fc:：millises（config:：block_interval_ms），uint32_t skip_flag=0/*skip_missed_block_penalty*/）override_
         auto sb=_产生_块（跳过_时间，错误，跳过_标志2）；
         auto bs=验证节点->创建块状态未来（sb）；
         验证_node->push_block（bs）；

         归还某人；
      }

      有符号的\u块\u ptr产生\u块\u无\u验证（fc：：微秒跳过\u时间=fc：：毫秒（config：：block \u interval \u ms），uint32 \u t跳过\u标志=0/*跳过\u丢失的\u块\u惩罚*/ ) {
      ...
      /*Ned_block_ptr产生_empty_block（fc:：microseconds skip_time=fc:：millises（config:：block_interval_ms），uint32_t skip_flag=0/*skip_missed_block_penalty*/）override_
         control->abort_block（）；
         auto sb=_product_block（skip_time，true，skip_flag_2）；
         auto bs=验证节点->创建块状态未来（sb）；
         验证_node->push_block（bs）；
      ...
      带符号的_block_ptr finish_block（）override_
         返回_finish_block（）；
      }
      ...
      bool validate（）
        ...
        auto-vn_hbh=验证_节点->head_block_state（）->header；
        ...
        正在验证_node.reset（）；
        验证_node=std:：make_unique<controller>（vcfg）；
        验证_node->add_indexs（）；
        正在验证_node->startup（[]（）return false；）；

        返回OK；
      }

      唯一的_ptr<controller>validating_node；
      uint32_t num_在_shutdown=0之前阻止_to_producer_；
      bool skip_validate=false；
   }；

   /**
    *实用程序谓词，用于检查fc:：exception消息是否等效于给定的字符串
    **/
  ...
  /*
   *用于检查fc:：exception消息是否以给定字符串开头的实用谓词
   **/
  ...
  /*
   *实用程序谓词，用于检查fc:：assert_异常消息是否等效于给定的字符串
   **/
  ...
  /*
   *实用程序谓词，用于检查fc:：assert_异常消息是否以给定字符串开头。
   **/
  ...
  /*
   *实用程序谓词，用于检查eosio_assert消息是否与给定字符串等效。
   **/
  ...
  /*
   *实用程序谓词，用于检查eosio_assert消息是否以给定字符串开头。
   **/
  ...
  /*
   *实用程序谓词，用于检查eosio断言代码错误代码是否等于给定的数字。
   **/
