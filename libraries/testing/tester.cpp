...
const fc::microseconds base_tester::abi_serializer_max_time{1000*1000}; //低速试验机1s
...
trx.max_net_usage_words = 0; //无限制
trx.max_cpu_usage_ms = 0; //无限制
...
return error(ex.top_message()); //top_message（）由许多测试假设；否则它们会失败。
//返回错误（例如to_detail_string（））；
      }
      produce_block();
