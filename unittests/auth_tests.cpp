...
BOOST_REQUIRE_THROW(push_reqauth(N(alice), "owner"), unsatisfied_authorization); //不带multisig
auto trace = push_reqauth(N(alice), "owner", true); //使用多重SIG

    produce_block();
    BOOST_REQUIRE_EQUAL(true, chain_has_transaction(trace->id));

} FC_LOG_AND_RETHROW() } ///缺少多个信号
...
chain.produce_blocks(13); //等待至少6秒钟，使第一个push-reqauth事务过期。

//更新“first”身份验证公钥
   chain.set_authority("alice", "first", second_pub_key, "active");
//权限已更新，使用以前的“第一个”身份验证在链接身份验证时应失败
...
//使用eosio创建帐户。带特权帐户的前缀
   chain.create_account("eosio.test1");

//使用eosio创建帐户。带有非特权帐户的前缀，应失败
...
///This should fail because spending is not active which is default for reqauth.这应该会失败，因为支出没有活动，这是reqauth的默认值。
   BOOST_REQUIRE_THROW( chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key }),
                        irrelevant_auth_exception );

   chain.produce_block();

//test.push_reqauth（n（alice），permission_u level_n（alice），“expending”，expending_priv_u key）；

   chain.link_authority( "alice", "eosio", "eosio.any", "reqauth" );
   chain.link_authority( "bob", "eosio", "eosio.any", "reqauth" );

///This should succeed because eosio：：reqauth is linked to any permission.（这应该成功，因为eosio:：reqauth链接到任何权限。）
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });

///Bob无法为Alice授权，因此此操作失败。给定的权限必须是Alice之一。
...
pls.push_back({acc1, "owner"}); //相同帐户，但权限名不同
...
.active   = invalid_auth//权限（chain.get_public_key（a，“active”），
                                });
