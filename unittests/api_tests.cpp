...
trx.context_free_data.emplace_back(fc::raw::pack<uint32_t>(100)); //验证负载是否与上下文无关的数据匹配
      trx.context_free_data.emplace_back(fc::raw::pack<uint32_t>(200));
      set_transaction_headers(trx);

//不允许只使用上下文\自由\操作对事务进行签名
//auto sigs=trx.sign（get_private_key（n（tesapi），“active”），control->get_chain_id（））；
...
//尝试在非上下文无关操作中访问上下文无关的API
      ...
      set_transaction_headers(trx);
//运行（dummy_action.b=200）案例，查找上下文自由API的无效使用
...
//契约应该被缓存，现在我们应该得到期限异常，因为从哈希函数调用checkTime（）。
        BOOST_CHECK_EXCEPTION( call_test( *this, test_api_action<TEST_METHOD("test_checktime", "checktime_sha1_failure")>{},
                                          5000, 10 ),
                               deadline_exception, is_deadline_exception );
...
//测试测试事务大小
CALL_TEST_FUNCTION(*this, "test_transaction", "test_transaction_size", fc::raw::pack(54) ); //托多：需要更好的方法来测试这个。

//测试测试读取事务
//这有点粗糙，但我找不到更好的方法来比较哈希值
...
//在没有替换现有标志的情况下计划两次（第二个延迟事务应替换第一个事务）
   {
      transaction_trace_ptr trace;
      uint32_t count = 0;
...
#warning re-enable deferred transaction replacement test after bug has been fixed
   #if 0
//使用replace_existing标志计划两次（第二个延迟事务应替换第一个事务）
   {
      transaction_trace_ptr trace;
      uint32_t count = 0;
      auto c = control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t && t->scheduled) { trace = t; ++count; } } );
...
//如果找不到计划事务，则cancel_deferred（）返回零
   {
...
//重复递延交易
   ...
   {
//触发一个Tx，该Tx反过来向付款人发送延迟Tx！=接收机
//付款人是Alice在这种情况下，这个Tx应该失败，因为我们没有Alice的授权
      dtt_action dtt_act1;
...
//发送一个Tx，然后发送一个带有延迟Tx接收器的延迟Tx！=此Tx接收器
//这将包括接管人的授权，并施加与管理局相关的任何延迟。
//我们在这里将授权延迟设置为10秒，并且由于延迟的Tx延迟设置为5秒，因此此Tx应该失败。
      dtt_action dtt_act2;
...
//但是如果延迟的事务有足够的延迟，那么它应该可以工作。
      dtt_act2.delay_sec = 10;
      CALL_TEST_FUNCTION(*this, "test_transaction", "send_deferred_tx_with_dtt_action", fc::raw::pack(dtt_act2));

//如果延迟的Tx接收器==此Tx接收器，则最初会绕过授权检查。
//但现在不行了。现在，由于testapi@eosio.code没有单方面满足testapi@additional权限的要求，它应该在主观上失败。
      dtt_action dtt_act3;
...
uint32_t lookup_type = 0; //0表示查找，1表示下界，2表示上界；
...
BOOST_FIXTURE_TEST_CASE(memory_tests, TESTER) { try {
   ...
   produce_blocks(1000);
//由于变量wasm max memory sizes的malloc更改而失败，因此暂时禁用此选项
#if 0
...
BOOST_CHECK_EQUAL( int64_t(0), get_result_int64() ); //由于签名不相关而失败
   ...
   /*
   boost_check_exception（调用_test_函数（*this，“test_permission”，“check_authorization”，
      fc:：raw：：pack（检查_auth_
         .account=n（测试程序），
         .permission=n（非名称），
         PUBKEY＝{
            获取公钥（n（tesapi），“active”）
         }
      ），fc：：异常，
       []（常量fc:：exception&e）
         返回expect_assert_message（e，“unknown key”）；
      }
   ；
   **/
