...
fc::microseconds max_serialization_time = fc::seconds(1); //有些试验机速度很慢

//验证通过字节进行的往返转换是否复制了完全相同的数据
...
//此测试导致使用fc_反射定义的打包执行的打包逻辑（因为
//packed_transaction:：data），与使用abi_序列化程序执行的解包逻辑结合，
//因此，非内置类型的ABI定义。此测试将公开任何事务和
//它的子类型在fc-reflect和abi-def中有不同的包装/拆包订单
...
//表示非常大的数组，但实际上不提供大数组
//curl http://127.0.0.1:8888/v1/chain/abi_bin_to_json-x post-d'“code”：“eosio”，“action”：“hi”，“binargs”：“ffffffff 08”
...
//注意：理想情况下，在验证结构“s2”的定义不正确时会拒绝此ABI。
//这种检查尚未在验证期间实现，但可以在序列化期间进行检查。
...
//此检查实际上指向当前ABI序列化程序的问题。
//选项数组（很遗憾，在验证中未拒绝）会导致此处出现解包异常，因为其中一个可选元素不存在。
//但是，abis.binary_to_variant（“s4”，fc:：variant（“0301010120103”）.as<bytes>（），max_serialization_time）将工作正常！
